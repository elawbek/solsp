    ...
    match req.method.as_str() {
        ...
        CallHierarchyPrepare::METHOD => {
            match req.extract::<CallHierarchyPrepareParams>(CallHierarchyPrepare::METHOD) {
                Ok((id, params)) => {
                    Response::new_ok(id, graphs::call_hierarchy_prepare(state, params))
                }
                Err(e) => extract_err_response(id, e),
            }
        }
        CallHierarchyIncomingCalls::METHOD => {
            ...
            {
                Ok((id, params)) => {
                    Response::new_ok(id, graphs::call_hierarchy_incoming(state, params))
                }
                Err(e) => extract_err_response(id, e),
            }
        }
        CallHierarchyOutgoingCalls::METHOD => {
            ...
            {
                Ok((id, params)) => {
                    Response::new_ok(id, graphs::call_hierarchy_outgoing(state, params))
                }
                Err(e) => extract_err_response(id, e),
            }
        }
        ...
        "solsp/inheritanceGraph" => {
            match req.extract::<serde_json::Value>("solsp/inheritanceGraph") {
                Ok((id, params)) => Response::new_ok(id, graphs::inheritance_graph(state, params)),
                Err(e) => extract_err_response(id, e),
            }
        }
        "solsp/functionCallGraph" => {
            match req.extract::<serde_json::Value>("solsp/functionCallGraph") {
                Ok((id, params)) => {
                    Response::new_ok(id, graphs::function_call_graph(state, params))
                }
                Err(e) => extract_err_response(id, e),
            }
        }
